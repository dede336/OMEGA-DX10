...
const LIGHT_DIGIMON_NO_GIF = new Set([
  ...
  'tailmon','salamon','angemon','magnaAngemon','goldramon','seraphimon',
]);
...
const PLANT_DIGIMON_NO_GIF = new Set([
  'palmon','togemon','lillymon','rosemon',
]);

const WIND_DIGIMON_NO_GIF = new Set([
  'magnadramon','phoenixmon','garudamon','birdramon','pyomon','patamon',
]);
...
export default function CharacterDetailScreen() {
  ...
  const isPlantNoGif         = PLANT_DIGIMON_NO_GIF.has(char.id);
  const isWindNoGif          = WIND_DIGIMON_NO_GIF.has(char.id);
  const hasSpecialGif        = isOmegamon || isShineGreymonBM || isRoseMonBM || isImperialDramonPM || isLightNoGif || isDarkNoGif || isFireNoGif || isPlantNoGif || isWindNoGif;
  const specialGif           = isOmegamon ? OMEGAMON_GIF
    ...
    : isPlantNoGif     ? PLANT_STATUS_GIF
    : isWindNoGif      ? WIND_STATUS_GIF
