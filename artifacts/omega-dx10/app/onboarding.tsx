...
export default function OnboardingScreen() {
  ...
  // Filter tamers: Neutral sees all, M/F see their group
  ...
  function handleNext() {
    if (step === 0) {
      ...
      if (trimmed.length < 2) { setNameError('O nome precisa ter pelo menos 2 caracteres.'); return; }
      if (trimmed.length > 16) { setNameError('O nome pode ter no máximo 16 caracteres.'); return; }
      setNameError('');
      animateStep(1);
    } else if (step === 1) {
      if (!gender) return;
      setTamerId(null); // reset tamer when gender changes
      animateStep(2);
    } else if (step === 2) {
      ...
      <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
        ...
        <Animated.View style={[styles.content, { opacity: fadeAnim }]}>
              ...
              <View style={styles.genderGrid}>
                {GENDER_OPTIONS.map((g) => {
                  ...
                  return (
                    <TouchableOpacity
                      key={g.id}
                      style={[styles.genderCard, selected && { borderColor: g.color, backgroundColor: g.color + '22' }]}
          ...
          {step === 2 && (
            <View style={styles.stepContainer}>
              ...
              <Text style={styles.stepSubtitle}>Quem vai estar ao seu lado no mundo digital?</Text>
              <ScrollView style={styles.tamerList} contentContainerStyle={styles.tamerListContent} showsVerticalScrollIndicator={false}>
                {visibleTamers.map((t) => {
                  const selected = tamerId === t.id;
                  return (
                    <TouchableOpacity
                      key={t.id}
                      style={[styles.tamerCard, selected && { borderColor: t.accentColor, backgroundColor: t.accentColor + '18' }]}
                    ...
                    >
                      ...
                      <View style={styles.tamerInfo}>
                        ...
                        <Text style={styles.tamerFullName}>{t.fullName}</Text>
        ...
        <View style={styles.footer}>
          ...
          >
            ...
            <Feather name={step < TOTAL_STEPS - 1 ? 'arrow-right' : 'zap'} size={18} color={canNext ? '#ffffff' : '#475569'} />
          </TouchableOpacity>
        </View>
      </View>
    </KeyboardAvoidingView>
...
const styles = StyleSheet.create({
  ...
  inputWrapper: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#111827', borderWidth: 1.5, borderColor: '#1e293b', borderRadius: 14, paddingHorizontal: 14, paddingVertical: 4, width: '100%' },
  inputIcon: { marginRight: 10 },
  input: { flex: 1, fontSize: 17, color: '#f1f5f9', paddingVertical: 14, fontWeight: '600' as const },
  ...
  genderCard: { flex: 1, alignItems: 'center', justifyContent: 'center', borderRadius: 16, borderWidth: 1.5, borderColor: '#1e293b', backgroundColor: '#111827', paddingVertical: 28, gap: 8, position: 'relative' as const },
  ...
  tamerAvatarWrapper: { width: 76, height: 76, borderRadius: 38, borderWidth: 2.5, overflow: 'hidden' as const, backgroundColor: 'transparent' },
  tamerAvatar: { width: '100%', height: 170, marginTop: -8 },
  ...
  nextBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 10, backgroundColor: '#6366f1', borderRadius: 16, paddingVertical: 18 },
